...
for setName in setNames:
    ...
    checksums = load_checksums_from_images(f'images/{setName}_corrected/')
    if dbg: print('Finished loading checksums')

    if dbg: print('Loading coordlist')
    with open(f'images/{setName}_corrected/_coordList.npy', 'rb') as f:
        coordList = np.load(f)
    if dbg: print('Finished loading coordlist')

    if dbg: print('Loading results')
    resultsDict = load_results_dict(f'output/output_{setName}_corrected.txt')
